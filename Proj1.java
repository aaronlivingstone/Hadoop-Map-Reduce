...
public class Proj1{
	...
	/*
		...
		*/
	public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> { // DoublePair
		...
		/** Actual map function. Takes one document's text and emits key-value
			...
			* @param docContents Text of the current document.
		...
		throws IOException, InterruptedException {
			...
			// YOUR CODE HERE
			Text word = new Text();
			Text targetWord = new Text(targetGram.toLowerCase());
			int counter = 1;
			ArrayList<Integer> targetList = new ArrayList<Integer>();
			DoublePair distance = new DoublePair();
			...
			while (matcher.find()) {
				...
				if (!(word.equals(targetWord))) {
					if (targetList.size() > 0) {
						distance.setDouble1((double)calcDistance(counter, targetList));
					} else {
						distance.setDouble1(Double.POSITIVE_INFINITY);
					}
					distance.setDouble1(func.f(distance.getDouble1()));
					distance.setDouble2(1.0);
					context.write(word, distance);	// word, distance
				}
			}	
		}
		
	  private int calcDistance(int counter, ArrayList<Integer> targetList) {
			int minDistance = Integer.MAX_VALUE;
			for (Integer targetIndex: targetList) {
				minDistance = Math.min(minDistance, Math.abs(counter - targetIndex));
			}
			return minDistance;
		}
	...
	public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
		...
		Context context) throws IOException, InterruptedException {
			...
			DoublePair output = new DoublePair(sw, aw);
	...
	public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
		@Override
			public void reduce(Text key, Iterable<DoublePair> values, 
		Context context) throws IOException, InterruptedException { //Iterable<DoublePair>
			...
			if (sw > 0.0) {
				cooccurrence = sw * (Math.pow(Math.log(sw), 3)) / aw;
			} else {
				cooccurrence = 0.0;
			}
			
			DoubleWritable output = new DoubleWritable(-1.0 * cooccurrence);
	...
	public static class Reduce2 extends Reducer<DoubleWritable, Text, DoubleWritable, Text> {
		...
		Context context) throws IOException, InterruptedException {
			...
			// YOUR CODE HERE	
			DoubleWritable test = new DoubleWritable(Math.abs(key.get()));
			
			for (Text value: values) {
				if (n < N_TO_OUTPUT){ 
					context.write(test, value);
					n++;
				}
			}
		}
